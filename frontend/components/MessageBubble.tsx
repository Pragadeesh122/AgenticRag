...
export default function MessageBubble({ message, isStreaming }: MessageBubbleProps) {
  ...
  return (
    <div
      className={`flex flex-col gap-3 animate-message-in ${isUser ? 'items-end' : 'items-start'}`}
    >
      ...
      {!isUser && isStreaming && !message.content && message.toolCalls.length === 0 && (
        <div className="flex items-center gap-1.5 py-1">
          <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" style={{ animationDelay: '0ms' }} aria-hidden="true" />
          <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" style={{ animationDelay: '150ms' }} aria-hidden="true" />
          <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" style={{ animationDelay: '300ms' }} aria-hidden="true" />
        </div>
      )}
    </div>
  );
